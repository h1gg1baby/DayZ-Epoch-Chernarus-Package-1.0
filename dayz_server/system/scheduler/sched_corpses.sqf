...
{if (isNil _x) then {_x = -1;}} forEach ["DZE_NPC_CleanUp_Time", "ai_cleanup_time"];
		...
		if (local _x && {_x isKindOf "CAManBase"}) then {
			if (_x isKindOf "zZombie_Base") then {
				...
				if (_x getVariable["bodyName",""] != "") then {
					...
					if ((DZE_NPC_CleanUp_Time != -1 && {diag_tickTime - _deathTime > (60 * DZE_NPC_CleanUp_Time)} && {_x getVariable["bodyName",""] == "NPC"}) || (ai_cleanup_time != -1 && (diag_tickTime - _deathTime > (60 * ai_cleanup_time)) && (_x getVariable["bodyName",""] == "mission_ai"))) then {
					...
					if ((diag_tickTime - _deathTime > 40 * 60) && {_x getVariable ["bodyName", ""] != "NPC"} && {_x getVariable ["bodyName", ""] != "mission_ai"}) then {
						if (_x getVariable["sched_co_fliesDeleted",false] or !dayz_enableFlies) then {
							// flies have been switched off, we can delete body
							_sound = _x getVariable ["sched_co_fliesSource", nil];
