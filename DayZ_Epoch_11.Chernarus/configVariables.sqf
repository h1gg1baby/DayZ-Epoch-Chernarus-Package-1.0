...
if (!isDedicated) then {
	...
	dayz_tameDogs = true; // Allow taming dogs with raw meat. Note dog behavior is experimental and buggy.
	...
	DZE_BuildingLimit = 500; // Maximum allowed objects per plot
	...
	*/
	...
	if (DZE_Service_Points) then {
		...
		DZE_SP_Classes = ["Map_A_FuelStation_Feed","Land_A_FuelStation_Feed","FuelPump_DZ","TK_WarfareBVehicleServicePoint_EP1"]; // service point classes, You can also use dayz_fuelpumparray by its self for all the default fuel pumps.
		DZE_SP_MaxDistance = 100; // maximum distance from a service point for the options to be shown
